...
The cancellation rate is computed by dividing the number of canceled (by client or driver) requests made by unbanned users 
...
/*
1. First filtered out valid client_ID and Driver_ID, and date between Oct 1, 2013 and Oct 3, 2013
2. count the number of cancelled rides, and count the number the total rides, take division and get cancellation rate, round to 2 digits
3. group by date
*/
...
from Trips
where Client_ID in 
(select Users_Id as client_ID from Users where banned = "No" and Role = "client") 
and Driver_Id in 
(select Users_Id as Driver_Id from Users where banned = "No" and Role = "driver")
